# Counter-Strike Event Manager

你也看CS？看腻了固有的比赛？等烦了主办方发布比赛？现在机会就在你眼前！

## Release Version 0.12

- 重做了主页面，加入 Windows 桌面程序项目的下载，并将原网页内容放入子页面。

## Beta Version 0.11

- 更新了不同窗口的适配。
- 修复了部分文字显示问题。

## Beta Version 0.10

- 全游戏界面文案统一英文化。
- 全量巡检并修复残留中文乱码问题（如“总奖金额”等异常文本）。
- 修复白日模式下办赛界面“举办城市”搜索框输入文字颜色。
- 调整赛事界面逻辑：赛事未结束时隐藏 `Standings` 板块。
- 修复赛事 `Standings` 表头中 `Δ VRS` 的显示异常。

## Beta Version 0.9

- 新增 ``LETUSGUESS`` 模块。
  - 主页面左下角新增悬浮入口按钮，点击进入模块页。
  - 模块页新增“预测”与“历史”两个板块。
  - 预测板块展示下一天赛程，并支持对单场比赛进行预测。
  - 新增预测子页面，支持选择胜者与整数金额投入（不超过当前资金）。
  - 预测后立即扣除投入资金，赛后自动结算。
  - 历史板块按从新到旧展示预测记录、返还比例与结算资金。
- 返还比例计算公式与流程：
  - 设两支战队的 VRS 积分分别为 $a$ 和 $b$。
  - 令 $c=\min\{1000,\min\{a,b\}-1\}$，$p=a-c$，$q=b-c$，$k=\frac{2pq}{p+q}$。
  - 令 $m=1.50-\ln\left(\frac{p}{k}\right)$，$n=1.50-\ln\left(\frac{q}{k}\right)$，并对 $m,n$ 保留两位小数。
  - 再令 $x=\min\{\max\{m,1.00\},15.00\}$，$y=\min\{\max\{n,1.00\},15.00\}$。
  - 则 $x,y$ 分别为两支战队的返还比例。
- 调整热度系数乘数 $k$ 的计算公式：
  - 顶级战队系数：$k=k\cdot 6$
  - 强队系数：$k=k\cdot 2.5$
  - 明星队系数：$k=k\cdot 9.87$

## Beta Version 0.8

- 修复 IEM / BLAST 在跨天后可能卡赛程不推进的问题。
- 办赛界面与赛事日历界面日期输入新增日历图标，可点击呼出日历选择日期。
- 优化调整了部分 UI。
- 热度计算新增阶段乘数 $r$，并更新公式为 $h=\sqrt{g\cdot k\cdot r}$：
  - 非 Playoffs：$r=1$
  - 四分之一决赛：$r=2$
  - 半决赛：$r=5$
  - 决赛：$r=20$

## Beta Version 0.7

- 引入战队身份体系：
  - 顶级战队：世界排名前 5。
  - 强队：世界排名第 6~20。
  - 明星队：指定名单（可与顶级/强队身份叠加）。
  - 明星战队名单：``Vitality, FaZe, Natus Vincere, FUT, Falcons, G2, Spirit, MOUZ, FURIA, Astralis, The MongolZ, Aurora, PARIVISION, TYLOO, Liquid, Lynn Vision, BC.Game``。
- 引入热度系统：
  - 按 VRS、赛事奖金与队伍身份系数计算每场比赛热度。
  - 热度公式：
    - 设两支战队的 VRS 积分分别为 $a$、$b$，赛事奖金为 $c$，令 $d=\frac{c}{2000000}$。
    - 原始热度为 $g=a\cdot b\cdot d^2$。
    - 比例系数 $k$ 初始为 $1$：
      - 两支队伍中每有 1 支顶级战队，则 $k=k\cdot 5$。
      - 两支队伍中每有 1 支强队，则 $k=k\cdot 1.5$。
      - 两支队伍中每有 1 支明星队，则 $k=k\cdot 8$。
    - 最终热度为 $h=\sqrt{g\cdot k}$。
  - 比赛结束后，在每场比赛卡片下部展示单场热度。
  - 赛事页展示本届赛事累计热度。
- 引入资金系统：
  - 初始资金为 $10000000$ 美元。
  - 办赛时从资金中扣除奖金，资金不足则无法创建赛事。
  - 赛事结束时，设本届赛事累计热度为 $h$，则资金回复 $h$ 美元。
- 新增白日/黑夜模式切换功能
  - 新增白日模式配色，地区标签颜色保持不变。
  - 默认为黑夜模式。
  
## Beta Version 0.6

- 战队页新增地区信息展示。  
- 参赛名单区分 `SEED` 与 `CSEMTV RANK`。  
- 含直邀分段的赛事在邀请名单页加入分段分割线。
- 瑞士轮同组对阵分配机制优化。
- 举办赛事新增限制
  - 新增同年同城同赛制禁重规则。
  - Major 新增限制：下一届邀请截止日必须晚于上一届总决赛日。  
- 战队体力每日自动回复。
- 瑞士轮布局调整。  
- 日期与“下一天”改为右上角固定浮窗。  
- 新增快捷键 `Shift + .` 推进下一天。
  - 新增长按 `Shift + .`（`>`）可快进的功能。

## Beta Version 0.5

- 新增榜单“接受邀请成功率”显示，并恢复体力值与邀请系统的联动。
- 合并 Ongoing Event 与 Formal Event 为统一“赛事日历”。
  - 新增“跟踪当天”勾选项。
- 改进 Major 规则与流程。
  - 限制 Major 每年最多举办两次（按邀请截止日期）。
  - 新增 ``EU`` / ``AM`` / ``AS`` 分地区名额邀请，并支持按上届 Major 成绩动态调整直邀名额。
  - 将 ``Challengers`` / ``Legends`` / ``Champions`` 统一更名为 ``Stage 1`` / ``Stage 2`` / ``Stage 3``。
  - 邀请预测页按地区拆分并区分三阶段名额；参赛名单新增阶段差异展示。
- 统一 ``EU`` / ``AM`` / ``AS`` 地区标签样式与配色。
- 每次进入“办赛”页时，邀请截止日期默认填充为当前日期的下一天。
- 将 ``CSTV`` 更新为 ``CSEMTV``。

## Beta Version 0.4

- 将输入字符串改为查找并设定城市。
  - 新增搜索与按字典序排列功能。
  - 添加了若干个 CS 比赛热门城市。
- 修复了网页界面小时样式超出边框的问题。

## Beta Version 0.3

- 修复了潜在的 bug。
- 将网页标题修改为 `CS-Manager`。

## Beta Version 0.2

- 修复了排名混乱与奖金分配错误的问题。
- 为页面增加了标题。
- 移除了体力值显示。

## Beta Version 0.1

- 发布地区系统，现在每支战队都有对应的地区。
  - 新增对于按地区筛选战队并排序的支持。
- 新增固定初始值，解决了初始值不同的问题。
- 将战队数量从原先的 88 支战队扩充至 100 支。

## Release Version 0.0

$2026-4-15$

发布了 Counter-Strike Event Manager。

链接至 [cs-manager.top](https://cs-manager.top)。

- 发布“排名”板块，模仿 VRS 积分进行排名的实时维护，同时维护了赏金系统，支持按照赏金排序。
- 发布时间（日期）系统。
- 发布“办赛”板块，现在可以创建比赛了。
  - 开放 Major、IEM、EWC、BLAST、EPL 五种赛制可供选择。
  - 开放 HIGHEST、Tier S、Tier 1、Tier 2、Tier 3、Tier Open 六种级别可供选择。
  - 开放邀请日期填写。
  - 根据前三项选择，自动设置并生成赛程、邀请名单、奖金池。
- 发布“赛事库”板块，可以查看所有已创办的赛事。
- 发布“Ongoing Event”、“Formal Event”板块，可以查看赛程。
- 发布赛事界面，可以查看实时赛况、邀请名单、最终榜单、单图得分、比赛日期等。
- 发布战队详情界面，在部分页面点击战队名称可以进入此页面。此页面包含该战队的 VRS 积分、世界排名、累计赏金、历史荣誉、参赛记录等。
